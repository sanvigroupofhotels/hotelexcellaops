...
export const confirmRazorpayPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // UAT — Financial Consistency: one shared payment-completion workflow for
    // FULL and PARTIAL payments (records payment, splits the Razorpay
    // convenience fee into a booking charge, recomputes totals via triggers).
    const { completeRazorpayCapture } = await import("./razorpay-completion.server");
    const res = await completeRazorpayCapture({
      supabaseAdmin,
      bookingId: (tok as any).booking_id,
      amountInr,
    ...
    return { ok: true, alreadyRecorded: res.alreadyRecorded };
  });
